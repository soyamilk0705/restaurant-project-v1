...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    @Query("select r from Reservation r left join r.user u where u.id = :userId and r.cancel = 0 and r.reservationDate > CURRENT_DATE")
    ...
    @Query("select count(r) from Reservation r left join r.user u where u.id = :userId and r.cancel = 0 and r.reservationDate > CURRENT_DATE")
    ...
    @Query("select r from Reservation r left join r.user u where u.phoneNumber like %:keyword%")
    Page<Reservation> findAllByPhoneNumber(@Param("keyword") String keyword, Pageable pageable);

    Page<Reservation> findAllByReservationDateContaining(String reservationDate, Pageable pageable);

    Page<Reservation> findAllByReservationTimeContaining(String reservationTime, Pageable pageable);

    Page<Reservation> findAllByDemandContaining(String demand, Pageable pageable);
